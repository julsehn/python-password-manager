...
class PopupHandler {
      ...
      if (tabs[0]) {
        ...
        chrome.tabs.sendMessage(tabs[0].id, message, (response) => {
          if (chrome.runtime.lastError) {
            ...
            this.showStatus(response?.success ? 'Credentials autofilled' : `Error: ${response?.error || 'Could not autofill'}`, response?.success ? 'success' : 'error');
          }
        });
      } else {
        this.showStatus('Error: Could not get active tab', 'error');
      }
    });
  }
      ...
      if (tabs[0]) {
        ...
        chrome.tabs.sendMessage(tabs[0].id, message, (response) => {
          if (chrome.runtime.lastError) {
            ...
            this.showStatus(response?.success ? 'Credentials saved to local app' : `Error: ${response?.error || 'Could not save credentials'}`, response?.success ? 'success' : 'error');
          }
        });
      } else {
        this.showStatus('Error: Could not get active tab', 'error');
  ...
  checkConnection() {
    chrome.runtime.sendMessage({
      ...
      this.showStatus(connected ? 'Connected to local app' : 'Local app disconnected', connected ? 'success' : 'error');
    });
  }
